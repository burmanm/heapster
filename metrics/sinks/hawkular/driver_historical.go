...
func (h *hawkularSink) GetMetric(metricName string, metricKeys []core.HistoricalKey, start, end time.Time) (map[core.HistoricalKey][]core.TimestampedMetricValue, error) {
	...
	for _, key := range metricKeys {
		...
		o := metrics.Filters(metrics.TypeFilter(typ), metrics.TagsFilter(tags))
		...
		if len(mds) < 0 || len(mds) > 1 {
			return nil, fmt.Errorf("Given metric query options did not result in unique metricId, %q, %q", metricName, key)
		}
		...
		for _, dp := range dps {
			mvs = append(mvs, h.datapointToMetricValue(dp))
		}
...
func (h *hawkularSink) getLabelTagValues(string labelKey, o ...metrics.Modifier) ([]string, error) {
	tags := make(map[string]string)
	tags[labelKey] = "*"
